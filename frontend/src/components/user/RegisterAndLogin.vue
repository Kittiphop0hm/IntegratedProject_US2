...
<div v-if="pathName === 'Register'" class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-6">
    <div class="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md">
          ...
          <div v-if="isSeller" class="relative mb-3 flex flex-col">
            ...
            >
              <div class="flex flex-col items-center justify-center pt-5 pb-6">
                ...
                <p class="text-center mb-2 text-sm text-slate-500 group-hover:text-slate-600">
                  <span class="font-semibold text-[12px]">{{ frontImageShowname.length > 0 ? frontImageShowname : 'Front side'}}</span>
                </p>
              </div>
            </label>
            <input
        ...
        <div v-if="isSeller" class="relative mb-3 flex flex-col">
            <p @click="deleteImage(backImageShowname)" v-if="backImageShowname.length > 0" class="absolute top-2 right-4 text-red-500 font-bold cursor-pointer hover:text-red-300">X</p>
            <label
              for="fileBack"
              class="itbms-upload-button flex flex-col items-center justify-center w-full h-32 border-2 border-slate-300 border-dashed rounded-xl cursor-pointer bg-slate-50 hover:bg-slate-100 transition-all duration-200 group"
            >
              <div class="flex flex-col items-center justify-center pt-5 pb-6">
                ...
                <p class="text-center mb-2 text-sm text-slate-500 group-hover:text-slate-600">
                  <span class="font-semibold text-[12px]">{{backImageShowname.length > 0  ? backImageShowname : 'Back side'}}</span>
                </p>
              </div>
            </label>
            <input
